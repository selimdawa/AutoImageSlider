        ...
        when (animation) {
            SliderAnimations.ANTICLOCKSPINTRANSFORMATION, SliderAnimations.CLOCK_SPINTRANSFORMATION -> {
                ...
                if (position in -1f..1f) {
                    ...
                    page.rotation = (if (position <= 0f) 360f else -360f) * factor
                }
            }

            SliderAnimations.DEPTHTRANSFORMATION -> {
                page.alpha =
                    if (position in -1f..1f) (if (position <= 0f) 1f else 1f - absPos) else 0f
            ...
            SliderAnimations.FADETRANSFORMATION -> {
                page.translationX = -position * page.width
                page.alpha =
                    if (position in -1f..1f) (if (position == 0f) 1f else if (position <= 0f) position + 1f else 1f - position) else 0f
            }

            SliderAnimations.FANTRANSFORMATION -> {
                page.translationX = -position * page.width
                page.pivotX = 0f; page.pivotY = (page.height / 2).toFloat()
                page.alpha = if (position in -1f..1f) 1f else 0f
                if (position in -1f..1f) page.rotationY =
                    (if (position <= 0f) -120f else 120f) * absPos
            }
            ...
            SliderAnimations.FIDGETSPINTRANSFORMATION -> {
                ...
                page.visibility = if (absPos < 0.5f) View.VISIBLE else View.GONE
                page.alpha = if (position in -1f..1f) 1f else 0f
                if (position in -1f..1f) page.rotation =
                    (if (position <= 0f) 36000f else -36000f) * (absPos * absPos * absPos * absPos * absPos * absPos * absPos)
            }

            SliderAnimations.HINGETRANSFORMATION -> {
                page.translationX = -position * page.width
                page.pivotX = 0f; page.pivotY = 0f
                page.alpha =
                    if (position in -1f..1f) (if (position <= 0f) 1f - absPos else 1f) else 0f
            ...
            SliderAnimations.TOSSTRANSFORMATION -> {
                ...
                page.alpha = if (position in -1f..1f) 1f else 0f
                if (position in -1f..1f) {
                    page.scaleX = max(0.4f, 1f - absPos); page.scaleY = max(0.4f, 1f - absPos)
            ...
            SliderAnimations.ZOOMOUTTRANSFORMATION -> {
                page.alpha = if (position in -1f..1f) max(0.3f, 1f - absPos) else 0f
                if (position in -1f..1f) {
                    page.scaleX = max(0.65f, 1f - absPos); page.scaleY = max(0.65f, 1f - absPos)
                }
            }
