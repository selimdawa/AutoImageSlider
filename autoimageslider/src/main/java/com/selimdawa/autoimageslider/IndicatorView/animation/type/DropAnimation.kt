...
class DropAnimation(listener: ValueController.UpdateListener) :
    ...
    private val value: DropAnimationValue = DropAnimationValue()
    ...
    override fun progress(progress: Float): DropAnimation {
        if (animator != null) {
            ...
            for (anim in animator!!.getChildAnimations()) {
    ...
    fun with(
        widthStart: Int, widthEnd: Int, heightStart: Int, heightEnd: Int, radius: Int
    ): DropAnimation {
        if (hasChanges(widthStart, widthEnd, heightStart, heightEnd, radius)) {
            ...
            val radiusForwardAnimator =
                createValueAnimation(radius, toRadius, halfDuration, AnimationType.Radius)
            ...
            val radiusBackwardAnimator =
                createValueAnimation(toRadius, radius, halfDuration, AnimationType.Radius)

            animator!!.play(heightForwardAnimator).with(radiusForwardAnimator).with(widthAnimator)
                .before(heightBackwardAnimator).before(radiusBackwardAnimator)
        }
    ...
    private fun createValueAnimation(
        fromValue: Int, toValue: Int, duration: Long, type: AnimationType
    ): ValueAnimator {
