    ...
    override fun transformPage(page: View, position: Float) {
        ...
        when (animation) {
            SliderAnimations.ANTICLOCKSPINTRANSFORMATION, SliderAnimations.CLOCK_SPINTRANSFORMATION -> {
                page.translationX = -position * page.width; page.visibility =
                    if (absPos < 0.5f) View.VISIBLE else View.GONE
                if (page.isVisible) {
                    page.scaleX = 1f - absPos; page.scaleY = 1f - absPos
                }
                page.alpha = if (inRange) 1f else 0f
                if (inRange) page.rotation =
                    (if (position <= 0f) 360f else -360f) * (if (animation == SliderAnimations.ANTICLOCKSPINTRANSFORMATION) 1f - absPos else absPos)
            }
            ...
            SliderAnimations.FANTRANSFORMATION -> {
                page.translationX = -position * page.width; page.pivotX = 0f; page.pivotY =
                    page.height / 2f; page.alpha = if (inRange) 1f else 0f
                if (inRange) page.rotationY = (if (position <= 0f) -120f else 120f) * absPos
            }

            SliderAnimations.FIDGETSPINTRANSFORMATION -> {
                page.translationX = -position * page.width; page.visibility =
                    if (absPos < 0.5f) View.VISIBLE else View.GONE; page.alpha =
                    if (inRange) 1f else 0f
                if (inRange) page.rotation =
            ...
            SliderAnimations.HORIZONTALFLIPTRANSFORMATION, SliderAnimations.SPINNERTRANSFORMATION, SliderAnimations.VERTICALFLIPTRANSFORMATION, SliderAnimations.VERTICALSHUTTRANSFORMATION -> {
                page.translationX = -position * page.width; page.visibility =
                    if (position in -0.5f..0.5f) View.VISIBLE else View.INVISIBLE; page.alpha =
                    if (inRange) 1f else 0f
                if (inRange) {
                    val rot =
            ...
            SliderAnimations.TOSSTRANSFORMATION -> {
                page.translationX = -position * page.width; page.visibility =
                    ...
                    if (inRange) 1f else 0f
                if (inRange) {
                    page.scaleX = max(0.4f, 1f - absPos); page.scaleY =
                        max(0.4f, 1f - absPos); page.translationY = -1000f * absPos
